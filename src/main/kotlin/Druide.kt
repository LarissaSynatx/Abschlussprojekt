class Druide(name: String, maxHp: Int, waffe: String, atk: Int, schild: Boolean, besiegt: Boolean) : Held(name, maxHp, waffe, atk, schild,besiegt) {
    ...
    fun schutzzauber() {
        println("$name beschwört mit seinem Schutzzauber ein Schild, bevor der nächste Angriff des Gegners ihn trifft und verringert damit den Schaden!")
        Thread.sleep(500)
        schild = true
    }
    ...
    fun angriff(endGegner: Endgegner, gegner: MutableList<Gegner>, helden: MutableList<Held>) {
        ...
        if (verflucht && hp < maxHp * 0.2) {
            verflucht = false
            endGegner.flüche = 0
            println("Der Fluch ist vorüber und verursacht keinen Schaden mehr")
            Thread.sleep(500)
            println("-- -- -- -- -- -- -- --")
            Thread.sleep(500)
        }
